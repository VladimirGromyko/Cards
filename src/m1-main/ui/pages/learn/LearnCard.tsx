...
export const LearnCard = ({
    ...
    const [option, setOption] = useState<LearnRadioValueType[]>(initualRadioValues)
    const [questionRate, setQuestionRate] = useState<QuestionRateType>({card_id: '', grade: 0})
    const checkAnswer = (i: number) => {
        currentCard._id && setQuestionRate({card_id: currentCard._id, grade: (i+1) as GradeType})
        const newRadioValues: LearnRadioValueType[] = initualRadioValues.map((item, index) => {
            return index ===  i ? {...item, selected: 1} : {...item, selected: 0}
        })
    ...
    return (
            <div className={s.wrapper}>
                <div className={s.question_wrapper}>
                    ...
                    <div className={s.shots}>{`Количество попыток ответов на вопрос: ${currentCard.shots}`}</div>
                </div>
                ...
                {showAnswer &&
                        ...
                        {option.map((el, i) => {
                          return (
                                  <span className={s.rate} key={el.grade} >
                                      <input type={"radio"} value={el.selected}
                                             checked={!!el.selected} className={s.rate}
                    ...
                    <SuperButton
                        ...
                        dis={true}
                        style={questionRate.grade || !showAnswer ? workButtonStyle : waitButtonStyle}
