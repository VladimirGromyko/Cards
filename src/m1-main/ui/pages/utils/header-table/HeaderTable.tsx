...
export const HeaderTable = ({ columArr, sorted, setSorting }: ColumnHeadingsType) => {
    const [isSorting, setIsSorting] = useState(false)
    const [columnHeadings, setColumnHeadings] = useState<HeadingsElementType[]>(columArr)
    ...
    const sortingOver = (e: React.MouseEvent<HTMLDivElement> | string, sortName: string) => {
        ...
        <>
            {columnHeadings.map((el, index) => {
                return (
                    ...
                    >{el.headings}
                        <SuperSorting sort={sorted} sorting={el.sortField} show={el.arrow} handleClick={() => mainSorting(el.sortField)}/>
                    </div>
                )
            })}
        </>
