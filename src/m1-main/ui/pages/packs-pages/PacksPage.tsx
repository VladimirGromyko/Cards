...
export const PacksPage = () => {
  ...
  const authorId = useAppSelector(packsSelector.authorId);
  ...
  const deletePackList = useCallback(
    (packId: string) => {
  ...
  // Block for Edit pack
  const editPackList = useCallback(
    (packName: string, privateStatus: boolean, packId: string) => {
      dispatch(
        editPackTC({
          cardsPack: { _id: packId, name: packName, private: privateStatus },
        })
      );
    },
    [dispatch]
