...
export const authAPI = {
  login: (payload: LoginType) => {
    return instance.post<LoginType, AxiosResponse<UserDataType>>(
      "auth/login",
      payload
    );
  },
  ...
  updateUser(payload: UserProfileType) {
    return instance.put<
      UserProfileType,
      AxiosResponse<UserProfileResponseType>
    >("auth/me", payload);
  },
  ...
  registrationUser(payload: RegistrationType) {
    return instance.post<RegistrationType, AxiosResponse<RegisterResponseType>>(
      `auth/register`,
      payload
    );
  },
...
export type RegisterResponseType = {
  addedUser: Omit<UserDataType, "token" | "tokenDeathTime">;
  // addedUser: {
  //   _id: string;
  //   email: string;
  //   rememberMe: boolean;
