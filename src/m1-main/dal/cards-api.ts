...
export type SortCardsHeaderType =
  | "question"
  | "answer"
  | "updated"
  | "grade"
...
export const cardsAPI = {
  getAllCards(params: {
    ...
    return instance.get(
      `/cards/card?cardsPack_id=${params.cardsPackId}&pageCount=${params.pageCount}&sortCards=${params.sortNumber}${params.sortName}&cardQuestion=${params.search}`
    );
  },
  getCards(param: { params: CardsGetRequestType }) {
    return instance.get<CardsGetRequestType, AxiosResponse<CardsSetType>>(
      "cards/card",
      param
    );
  },
  addCard(param: { card: PostRequestCardType }) {
    return instance.post<PostRequestCardType, AxiosResponse>(
      `/cards/card`,
      param
    );
  },
  ...
  updateCard(params: PutRequestUpdateCardType) {
    return instance.put<PutRequestUpdateCardType, AxiosResponse>(
      `/cards/card`,
      { card: params }
    );
  },
  gradeCard: (params: GradeCardPayload) => {
    return instance.put<GradeCardPayload, AxiosResponse<any>>(
      "/cards/grade",
      params
    );
  },
